...
def read_criteria(ws, base_row: int, max_search: int = 6) -> dict:
    """
    base_row の直下から判断基準テキストを読み込む。
    COL_CRITERIA(col5) = テキスト, COL_SKILL_SCORE(col7) = 点数値（1〜5）
    戻り値: {5: "text", 4: "text", ...}
    """
    criteria = {}
    for r in range(base_row + 1, base_row + max_search + 1):
        text = ws.cell(r, COL_CRITERIA).value
        pts  = ws.cell(r, COL_SKILL_SCORE).value
        if text and pts and isinstance(pts, (int, float)):
            pt = int(float(pts))
            if 1 <= pt <= 5:
                criteria[pt] = str(text)
    return criteria

def read_skill_ws(ws) -> tuple:
    skill = {}
    for name, row, coeff, kind in SKILL_ITEMS:
        skill[name] = {
            "score": safe_int(ws.cell(row, COL_SKILL_SCORE).value),
            "coeff": coeff,
            "kind":  kind,
        }
    cf = {}
    for name, row, coeff, group in CF_ITEMS:
        cf[name] = {
            "score": safe_int(ws.cell(row, COL_SKILL_SCORE).value),
            "coeff": coeff,
            "group": group,
...
def read_all_criteria(ws) -> tuple:
    skill_criteria = {name: read_criteria(ws, row) for name, row, _, _ in SKILL_ITEMS}
    cf_criteria    = {name: read_criteria(ws, row) for name, row, _, _ in CF_ITEMS}
    return skill_criteria, cf_criteria
...
def parse_skill_file(file_bytes: bytes, filename: str = "") -> dict:
    ...
    curr_skill, curr_cf = read_skill_ws(ws_curr)
    prev_skill, prev_cf = read_skill_ws(ws_prev) if ws_prev else ({}, {})
    skill_criteria, cf_criteria = read_all_criteria(ws_curr)

    s, c   = calc_skill_totals(curr_skill, curr_cf)
...
def render_skill_comparison(
    ...
    summary = {
        m: {
            "職種":         st.session_state.skill_members[m]["job_type_label"],
            "スキルスコア": st.session_state.skill_members[m]["skill_total"],
            "CFスコア":     st.session_state.skill_members[m]["cf_total"],
            "合計":         st.session_state.skill_members[m]["total"],
            "等級":         st.session_state.skill_members[m]["grade"],
        }
        for m in members
    }
    st.dataframe(
        pd.DataFrame(summary).T.style.highlight_max(
            subset=["スキルスコア", "CFスコア", "合計"], color="#d4edda"
        ),
...
def score_to_rank(total_score: float, weight_sum: float) -> str:
    """獲得スコアとウェイト合計から達成率で目安評価を計算（Excel準拠）"""
    if weight_sum <= 0:
        return "–"
...
RANK_COLOR = {
    "S": "#1565C0", "A+": "#1976D2", "A": "#1E88E5", "A－": "#42A5F5",
    "B+": "#388E3C", "B": "#43A047", "B－": "#66BB6A",
    "C+": "#F57F17", "C": "#FB8C00", "C－": "#FFA726",
    "D": "#B71C1C", "–": "#9E9E9E",
}
    ...
    if not st.session_state.target_members:
        ...
        if selected:
            ...
            if view_mode == "✏️ 下期UL評価入力":
                ...
                st.caption("※ 目安評価はExcel準拠（達成率135%超=S、115%超=A、98〜102%=B、65%以下=D など）")
