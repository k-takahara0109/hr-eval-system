...
def parse_skill_file(file_bytes: bytes, filename: str = "") -> dict:
    ...
    # ── シート識別ロジック ──────────────────────────────────────
    # 「等級要件」セル（row2,col2）を持つシートだけを対象にする
    # シート名に YYYYMM が含まれる場合は日付で降順ソート（最新=今期）
    # どのシートにも年月がない場合は左端を今期、次を前期とする

    def _yyyymm(name: str) -> int:
        import re
    ...
    for s in sheets:
        try:
            ...
            if "等級要件" in val:
                skill_sheets.append(s)
        except:
            pass

    # 等級要件シートが見つかった場合 → 日付ソートで今期/前期を判定
    if skill_sheets:
        skill_sheets = sorted(skill_sheets, key=_yyyymm, reverse=True)
        ws_curr = wb[skill_sheets[0]]
        ws_prev = wb[skill_sheets[1]] if len(skill_sheets) > 1 else None
    else:
        # 等級要件セルが見つからない場合は位置で判定（フォールバック）
        ws_curr = wb[sheets[0]]
