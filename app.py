        ...
        for r in range(start, end):
            ...
            if is_item_header(c5):
                ...
                if cur_name and cur_name != item_name and cur_name not in items:
                    items[cur_name]    = {"score": cur_score, "coeff": cur_coeff, "kind": "", "row": cur_row}
...
def export_skill_excel(member_data: dict) -> bytes:
    ...
    # 動的キーでスキル項目を書き戻す
    for name, info in d["skill_scores"].items():
        row = info.get("row")
        if row is None:
            continue
        new_score  = info["score"]
        coeff      = info.get("coeff", 1)
    ...
    # 動的キーでCF項目を書き戻す
    for name, info in d["cf_scores"].items():
        row = info.get("row")
        if row is None:
            continue
        new_score  = info["score"]
...
def render_skill_comparison(
    ...
    """
    スキル評価カード型UI（動的項目キー対応版）。
    メンバー間で項目セットが異なる場合（営業 vs ディレクター等）も正しく表示する。
    """
    ...
    for m in members:
        ...
        item_sets[m] = set(d.get(score_key, {}).keys())

    unique_sets = list({frozenset(s) for s in item_sets.values()})
    is_homogeneous = len(unique_sets) <= 1

    if not is_homogeneous:
        # ── 異なる項目セットのメンバーが混在 → サマリー表示のみ ──
        st.warning(
            "⚠️ 職種が異なるメンバーが含まれています。"
            "メンバーごとに項目が異なるため、サマリーのみ表示します。"
            "スコア編集は**職種タブ**から行ってください。"
        )
        _render_score_summary(members, score_key)
        return

    # ── 以降は同一項目セットのメンバー向け（従来の比較グリッド表示） ──
    SCORE_OPTS = [0, 1, 2, 3, 4, 5]

    first = st.session_state.skill_members[members[0]]
    dynamic_items = list(first.get(score_key, {}).keys())
    all_criteria  = first.get(criteria_key, {})

    if not dynamic_items:
        st.info("この職種の項目データがありません。")
        return
    ...
    # ── 項目カードを1行ずつ描画（動的キーで参照）─────────────
    for item_name in dynamic_items:
        item_data = first[score_key].get(item_name, {})
        coeff     = item_data.get("coeff", 1)
        ...
        for m in members:
            d = st.session_state.skill_members[m]
            cur_scores[m] = d[score_key].get(item_name, {}).get("score", 0)
    ...
    if col_btn.button("💾 スコアを保存", key=f"save_btn_{ver_key}", type="primary"):
        changes = []
        first_m = members[0]
        dynamic_keys = list(st.session_state.skill_members[first_m].get(score_key, {}).keys())
        for item_name in dynamic_keys:
            for m in members:
...
def _render_score_summary(members: list, score_key: str = None):
    """スコアサマリーテーブルを描画する（共通ヘルパー）"""
    st.divider()
